...
const EditPurchaseOrderScreen = ({ navigation, route }) => {
    ...
    const [items, setItems] = useState(
        order.items.map(i => ({
            product: i.product?._id || i.product || null,
            stoneName: i.stoneName,
            quantityInSqFt: String(i.quantityInSqFt),
    ...
    const selectProduct = (product) => {
        setItems(prev => {
            ...
            updated[pickerTarget] = {
                ...
                quantityInSqFt: updated[pickerTarget].quantityInSqFt,
    ...
    const addItem = () => setItems(prev => [...prev, { product: null, stoneName: '', quantityInSqFt: '', unitPrice: '' }]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await api.put(`/purchase-orders/${order._id}`, payload);
            Alert.alert('Success', 'Purchase order updated successfully.', [
                { text: 'OK', onPress: () => navigation.goBack() },
            ]);
        } catch (error) {
            Alert.alert('Error', error.response?.data?.message || 'Failed to update purchase order');
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <SafeAreaView style={styles.container} edges={['top']}>
            <View style={styles.header}>
                ...
                <View>
                    ...
                    <Text style={styles.headerSub}>{order.poNumber}</Text>
                </View>
                <View style={{ width: 24 }} />
            </View>
...
const styles = StyleSheet.create({
    ...
    headerSub: { color: COLORS.teal, fontSize: 12, fontWeight: '600', textAlign: 'center' },
